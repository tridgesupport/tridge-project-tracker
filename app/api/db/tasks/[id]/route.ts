...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { _changes, _editedByEmail, _notifyEmail, _notifyName, _taskName, _projectName, _projectId, ...body } = await req.json()

  await sql`
    UPDATE tasks SET
      task_name = COALESCE(${body.task_name ?? null}, task_name),
      description = ${body.description !== undefined ? body.description : null},
      comments = ${body.comments !== undefined ? body.comments : null},
      links = ${body.links !== undefined ? body.links : null},
      documentation_link = ${body.documentation_link !== undefined ? body.documentation_link : null},
  ...
  if (_changes && _editedByEmail && Object.keys(_changes).length > 0) {
    await sql`
      ...
      VALUES ('task', ${id}, ${_editedByEmail}, now(), ${JSON.stringify(_changes)})`
  }
  ...
  if (_notifyEmail && _projectId) {
    fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/send-email`, {
      ...
      body: JSON.stringify({
        toEmail: _notifyEmail, toName: _notifyName, entityType: 'Task',
        entityName: _taskName, projectName: _projectName,
