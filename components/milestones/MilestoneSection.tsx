...
import { getComments, createMilestone, updateMilestone, deleteMilestone, updateTask } from '@/lib/api'
...
export function MilestoneSection({
  ...
  async function loadComments() {
    ...
    try {
      const data = await getComments(allIds)
      setComments(data)
    } catch (err) {
  ...
  async function saveTaskEdit(taskId: string, milestoneId: string, field: string, value: unknown) {
    ...
    const changes = { [field]: { old: (task as any)[field], new: value } }
    const notifyUser = field === 'next_action_by' && value && value !== task.next_action_by
      ? internalUsers.find(u => u.id === value)
      : null
  ...
  async function saveMilestone() {
    if (!milestoneForm.milestone_name.trim()) { toast.error('Milestone name is required'); return }
    setSaving(true)

    const now = new Date().toISOString()
    const notifyUser = milestoneForm.next_action_by && milestoneForm.next_action_by !== editingMilestone?.next_action_by
      ? internalUsers.find(u => u.id === milestoneForm.next_action_by)
      : null
    ...
    try {
      if (editingMilestone) {
        ...
        if (data?.id) setExpanded(prev => new Set([...prev, data.id]))
  ...
  return (
    <div>
        ...
        <div className="border-2 border-primary/20 rounded-lg p-4 mb-4 bg-muted/20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="sm:col-span-2 flex flex-col gap-1">
              ...
              <Input value={milestoneForm.milestone_name}
                onChange={e => set('milestone_name', e.target.value)}
                placeholder="e.g. Phase 1 — Data Collection" autoFocus />
            </div>
            <div className="sm:col-span-2 flex flex-col gap-1">
              <Label className="text-xs font-medium">Description</Label>
              <Textarea value={milestoneForm.description}
                onChange={e => set('description', e.target.value)} rows={2} placeholder="Optional description" />
            </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              <Select value={milestoneForm.assigned_to ?? NONE}
            ...
            <div className="flex flex-col gap-1">
              ...
              <Select value={milestoneForm.next_action_by ?? NONE}
            ...
            <div className="flex flex-col gap-1">
              ...
              <Select value={milestoneForm.priority !== null ? String(milestoneForm.priority) : NONE}
                    ...
                    {canEdit && (
                      <TableCell onClick={e => e.stopPropagation()}>
                        <div className="flex items-center gap-1">
                          ...
                          <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive" onClick={() => handleDeleteMilestone(m.id)}>
                            <Trash2 size={13} />
                          </Button>
                        </div>
                      </TableCell>
                                    ...
                                    <TableRow className="hover:bg-muted/50">
                                      <TableCell className="font-medium">
                                        {isTaskEditing(t.id, 'task_name') ? (
                                          <Input autoFocus className="h-7 text-sm min-w-[160px]"
                                      ...
                                      <TableCell className={canEdit ? 'cursor-pointer text-xs' : 'text-xs'} onClick={e => startTaskSelectEdit(e, t.id, 'priority')}>
                                        {isTaskEditing(t.id, 'priority') ? (
                                          <Select open onOpenChange={open => { if (!open) setTaskEditingCell(null) }}
                                            value={t.priority !== null ? String(t.priority) : NONE}
                                            onValueChange={v => saveTaskEdit(t.id, m.id, 'priority', v === NONE ? null : Number(v))}>
                                        ...
                                        <TableCell colSpan={10} className="p-0 bg-muted/10 border-t-0">
                                          <div className="px-6 py-3 border-b">
                                            <CommentThread
                                              ...
                                              canComment={canEdit} onCommentAdded={loadComments}
                                            />
                                          </div>
                                        </TableCell>
                                      </TableRow>
      ...
      {taskModal && (
        <TaskModal
          open onClose={() => setTaskModal(null)}
